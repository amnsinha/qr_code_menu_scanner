...
const CHEF = '/chef';
const GET_NEW_ORDERS = BASE + CHEF + '/get-new-orders';
...
export class OrderService extends BaseService {
  ...
  public getCustomerNewOrders(token: string, outletId: string): Observable<any> {
    const url: string = this.getServiceUrl(GET_NEW_ORDERS)
      .concat('/').concat(token)
      .concat('/').concat(outletId);
    return this.httpClient.get<any>(url);
  }

  public getProcessedOrders(token: string, outletId: string): Observable<any> {
